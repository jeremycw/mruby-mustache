...
#ifdef CRUSTACHE_CUSTOM_ALLOCATION
	...
	struct stack *context)
{
	...
	case CRUSTACHE_VAR_STR:
		...
		case CRUSTACHE_TAG_ESCAPE:
			houdini_escape_html(ob, tag_value.data.u.cstr, tag_value.size);
			break;

		case CRUSTACHE_TAG_UNESCAPE:
			houdini_unescape_html(ob, tag_value.data.u.cstr, tag_value.size);
			break;

		case CRUSTACHE_TAG_RAW:
			bufput(ob, tag_value.data.u.cstr, tag_value.size);
			break;
		}
		break;
	...
	if (node->inverted) {
		...
		case CRUSTACHE_VAR_LAMBDA:
		{
			...
			if (lambda_result.type == CRUSTACHE_VAR_STR) {
				bufput(ob, lambda_result.data.u.cstr, lambda_result.size);
			} else {
